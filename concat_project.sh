...
PRUNE_DIRS=( .git .venv __pycache__ .pytest_cache .ruff_cache logs sandbox pending_writes )
EXCLUDED_NAMES=( ".env" "poetry.lock" "*.png" "*.jpg" "*.jpeg" "*.gif" "*.webp" "*.ico" "*.pyc" "*.so" "$(basename "$0")" "$OUTPUT_FILE" )

# build prune expression: any depth directories with those names
prune_expr=()
...
| while IFS= read -r -d '' file; do
    ...
    if [[ -s "$file" ]]; then cat "$file" >> "$OUTPUT_FILE"; else printf "[EMPTY FILE]\n" >> "$OUTPUT_FILE"; fi
    printf "\n--- END OF FILE %s ---\n\n" "$file" >> "$OUTPUT_FILE"
  done
