...
*2026-07-19: claim 7 corrected against `main` — its prior mechanism (`require_role("platform_admin")` / JWT / HTTP 403) never matched the OSS runtime, where `require_governor` is a trusted-localhost pass-through (`src/api/dependencies.py:53`). Surfaced during #798 scoping. Other rows not re-run this session.*
