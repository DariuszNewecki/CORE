...
| 7 | Governor-only API routes are enforced at the FastAPI dependency layer, not merely declared in metadata. | `require_governor = require_role("platform_admin")` is defined at `src/api/dependencies.py:101`. It reads the JWT role claim and raises HTTP 403 if the caller is not `platform_admin`. Proposal creation, approval, rejection, and execution routes all declare `Depends(require_governor)` — the dependency is evaluated before the handler body runs. | A request to `POST /v1/proposals` without a `platform_admin` JWT returns 403. The handler body is never reached. | `curl -s -o /dev/null -w "%{http_code}" -X POST http://localhost:8000/v1/proposals -H "Content-Type: application/json" -d '{}'` — expect `401` (no token) or `403` (wrong role). |
