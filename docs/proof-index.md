...
This page maps each claim to the mechanism that enforces it, the evidence a reviewer can observe, and a command that produces that evidence on a running instance. The page is intentionally short — ten claims at v1, all verified against the codebase on the date of last update.
...
|---|---|---|---|---|
| 1 | `.intent/` is reached through a single gateway. | `IntentRepository` is the `.intent/` read facade (constitutional — CLAUDE.md rule 6). Its non-decomposition is declared in-file via `CORE_ROLE = "facade"` (`src/shared/infrastructure/intent/intent_repository.py:37`, ADR-095 D3) — the modularity engine short-circuits on that marker; the prior per-file `governed_exclusions` register was retired by ADR-095 D3. Direct `.intent` access outside the gateway is governed by `architecture.namespace.no_direct_protected_access` and `architecture.intent.gateway_is_shared_infrastructure` (`.intent/rules/architecture/intent_access.json`). | `intent_repository.py` carries `CORE_ROLE = "facade"`; the two `intent_access` rules are present in the ruleset. | `grep -n 'CORE_ROLE' src/shared/infrastructure/intent/intent_repository.py` — expect `CORE_ROLE = "facade"`. |
...
| 7 | Every governor-only / sensitive API route carries the `require_governor` enforcement hook at the FastAPI dependency layer, and that placement is constitutionally enforced — not merely declared in metadata. | `require_governor` is applied per-route as a dependency (e.g. `src/api/v1/proposals_routes.py` — create/approve/reject/execute each list `dependencies=[require_governor]`). In OSS mode it resolves to a **trusted-localhost pass-through** (`require_governor = Depends(_oss_passthrough)`, `src/api/dependencies.py:53`) — authentication is not the OSS threat model; core-platform mounts a real role guard at the *same* hook in Console mode. Presence of the hook on every governor-only/sensitive route is enforced by `architecture.api.router_exposure_must_match_dependencies` (blocking) and `architecture.api.sensitive_route_must_be_gated` (reporting), mapped in `.intent/rules/architecture/layer_separation.json`. | Every mutating proposal route declares `require_governor`; the two placement rules report zero ungated sensitive routes. | `grep -n "require_governor" src/api/v1/proposals_routes.py` — the dependency appears on each mutation route. |
| 8 | An autonomous commit cannot include files outside the action's declared production set. | `GitService.commit_paths()` at `src/shared/infrastructure/git_service.py:238` implements a two-layer guard (ADR-129 D1): it checks the staging area for paths outside the declared set and raises `StagingContaminationError` if any are found; it then commits with a pathspec-restricted `git commit -m <msg> -- <p1> <p2> …` so even a race between the check and the add cannot sweep in extra files. `StagingContaminationError` routes the proposal to `mark_failed`, not `mark_completed`. | `grep -n "StagingContaminationError\|git commit.*--" src/shared/infrastructure/git_service.py` shows the guard and pathspec commit on adjacent lines. | `grep -n "StagingContaminationError\|-- \$" src/shared/infrastructure/git_service.py` |
| 9 | No code path may write `.intent/` directly; vocabulary regeneration outputs a draft for governor review. | `core-admin intent sync vocabulary --stage` writes to `var/drafts/META/vocabulary.json` via `FileHandler` (ADR-130 D2). The constitutional guard that prevents runtime writes to `.intent/` (`governance.constitution.read_only`) is enforced by the `glob_gate` engine (`patterns_prohibited: .intent/**`, scoped to `src/**/*.py`, blocking) — a source file writing to a path under `.intent/` is a blocking violation. The governor applies the draft with a manual `cp`. | Running `--stage` leaves `.intent/META/vocabulary.json` untouched and produces `var/drafts/META/vocabulary.json`. No source file in `src/` contains a write call targeting `.intent/`. | `core-admin intent sync vocabulary --stage && ls -la var/drafts/META/vocabulary.json && git diff --name-only .intent/` — the diff must be empty. |
| 10 | Blackboard sensors cannot post outside their declared artifact type and rule namespace; duplicate `indeterminate` observations are refused. | `BlackboardPublisher.post_artifact_finding()` at `src/shared/workers/blackboard_publisher.py:110` validates `artifact_type` against the worker's declared `mandate.scope.artifact_type` and `sub_namespace` against the declared `rule_namespace` before any INSERT. A mismatch raises immediately. `post_observation()` at line 197 queries for an existing `indeterminate` row with the same subject before inserting; a duplicate is refused with a logged warning rather than silently overwriting. | Source inspection shows the validation guard on lines 120–151 and the duplicate-refusal query on lines 206–215. | `grep -n "not in declared\|post_observation refuses" src/shared/workers/blackboard_publisher.py` |
...
async def main():
    async with get_session() as s:
        rows = (await s.execute(text(\"\"\"
            SELECT action_type, ok, agent_id, created_at
            FROM core.action_results
            WHERE agent_id = 'ActionExecutor'
...
Each row is one governed atomic-action execution. Reviewer should observe: with the `agent_id='ActionExecutor'` filter every row is a governed action-execution record, `action_type` matches a registered action ID, and the trail accumulates to the present (the latest row is recent). The shared table also holds other producers' evidence (`test_runner_infra` pytest rows, legacy `alignment_orchestrator`); the filter isolates the governed-action ledger. Writer exclusivity over the raw table is not claimed.
...
V1 covers ten claims chosen for what they prove together: a single read gateway (1), no executor bypass (2), no layer bypass at the API surface (3), no untracked mutation (4), no auto-execution of a dangerous action without approval (5), end-to-end causal traceability from finding to file change (6), governor-only API enforcement at the dependency layer (7), autonomous commit authorship integrity (8), `.intent/` immutability enforced through staged regeneration (9), and blackboard publisher namespace constraints (10). Other claims worth adding — autonomous test-loop honesty, symbol-level test gap convergence — are deferred until the cited mechanisms stabilize.

If you find a row whose command no longer produces the cited evidence, the disagreement is the bug, not the test.

---

*Last verified: 2026-06-29 against `main`. Claims 7–10 added; claims 1–6 not re-run this session — verify independently if auditing the full set.*

*2026-07-19: claims 1 and 7 corrected against `main`, surfaced during #798 scoping. Claim 1's prior mechanism (a `governed_exclusions` entry) was retired by ADR-095 D3 in favour of the in-file `CORE_ROLE = "facade"` marker. Claim 7's prior mechanism (`require_role("platform_admin")` / JWT / HTTP 403) never matched the OSS runtime, where `require_governor` is a trusted-localhost pass-through (`src/api/dependencies.py:53`). Other rows not re-run this session.*
