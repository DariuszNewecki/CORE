...
CREATE TABLE core.repo_artifacts (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT repo_artifacts_type_check CHECK ((artifact_type = ANY (ARRAY['adr'::text, 'doc'::text, 'infra'::text, 'intent_json'::text, 'intent_yaml'::text, 'paper'::text, 'prompt'::text, 'python'::text, 'report'::text, 'spec_markdown'::text, 'test'::text])))
);
