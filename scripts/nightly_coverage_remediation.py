...
1. Checks if it is within its allowed operational time window.
2. Analyzes the codebase to find files with low test coverage.
3. For each low-coverage file, it identifies "SIMPLE" functions as candidates.
4. It creates a prioritized queue of files to fix.
5. It invokes the `core-admin coverage remediate --file` command for each
...
# --- Configuration ---
# The agent is allowed to work between these hours (24-hour format).
START_HOUR = 22  # 10 PM
END_HOUR = 7  # 7 AM
MINIMUM_COVERAGE_TARGET = 75  # The constitutional minimum.


def _is_within_operating_window() -> bool:
    """Checks if the current time is within the allowed window."""
    current_hour = datetime.now().hour
    # This handles overnight windows correctly (e.g., 22:00 to 07:00)
...
def _get_low_coverage_files() -> list[tuple[str, float]]:
    ...
    """
    console.print("[bold cyan]Step 1: Analyzing current test coverage...[/bold cyan]")
    ...
    # Filter out files that already meet the target and sort by coverage
    low_coverage = [
        (path, percent)
...
def _find_next_target(low_coverage_files: list[tuple[str, float]]) -> Path | None:
    """
    Finds the best next file to work on by looking for one with at least
    one "SIMPLE" test target.
    """
    console.print(
...
def _run_remediation_for_file(file_path: Path) -> bool:
    ...
    try:
        # Use subprocess.run and stream the output
...
def main():
    ...
    console.print(
        Panel(
            f"[bold green]🚀 Starting Nightly Autonomous Coverage Remediation[/bold green]\n"
    ...
    while _is_within_operating_window():
        ...
        console.print(
            f"\n[bold]--- Cycle {files_processed} complete. Checking time and looking for next target... ---[/bold]\n"
        )
    else:
        # This block runs if the `while` loop condition becomes false
        console.print(
    ...
    console.print(
        Panel(
            f"[bold green]✅ Nightly Remediation Run Finished. Processed {files_processed} file(s).[/bold green]",
            expand=False,
        )
    )
