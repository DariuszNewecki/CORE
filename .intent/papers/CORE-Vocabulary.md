...
| Crate | A staged, sandboxed package of changes. The unit of governed mutation. | *(paper needed)* |
| Gate | A validation point that must pass before execution continues. | *(paper needed)* |
...
| Remediation | Resolution of a Finding by applying a governed fix. | *(paper needed)* |
| Convergence | The state where Finding resolution exceeds Finding creation. The operational goal. | *(paper needed)* |
...
| IntentGuard | The runtime Gate that evaluates every file write against constitutional Rules. | *(paper needed)* |
| Canary | The execution Gate that validates a Crate before it is applied. | *(paper needed)* |
| ConservationGate | The Gate that ensures LLM-produced code preserves the logic it replaces. | *(paper needed)* |
| ConstitutionalEnvelope | The set of Rules injected into an LLM prompt to constrain its output. | *(paper needed)* |
...
The following concepts are used in CORE but have no authoritative paper yet:

- Crate
- Gate (and its implementations: IntentGuard, Canary, ConservationGate)
- Remediation
- Convergence
- ConstitutionalEnvelope

These are the gaps. Each needs a paper before the concept has constitutional standing.
