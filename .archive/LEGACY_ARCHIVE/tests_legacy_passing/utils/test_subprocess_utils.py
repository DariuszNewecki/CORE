...
from shared.utils.subprocess_utils import SubprocessCommandError, run_poetry_command
...
class TestRunPoetryCommand:
    ...
    def test_poetry_not_found_raises_exit(self):
        """Test that missing poetry executable raises SubprocessCommandError."""
        with patch("shutil.which", return_value=None):
            with pytest.raises(SubprocessCommandError) as exc_info:
                run_poetry_command("Test", ["pytest"])

            assert exc_info.value.exit_code == 1

    def test_command_failure_raises_exit(self):
        """Test that failed command raises SubprocessCommandError."""
        with patch("shutil.which", return_value="/usr/bin/poetry"):
            with patch("subprocess.run") as mock_run:
