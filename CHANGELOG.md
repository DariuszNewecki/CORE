...
- **Atomic migrations (D7).** `core-admin database migrate --write` runs each migration's
  ...
  re-reading the ledger inside the lock: a failure leaves that migration rolled back and
  unrecorded, earlier ones recorded, re-running safe; concurrent invocations apply each
...
- **Manifest completeness (D6, D12).** `20260722_active_finding_dedup.sql` and
  ...
  loader refuses otherwise. Every entry after v2.9.1 carries a `verify` probe; only the two
  hand-applied files and the ledger column are `reconcilable` (recorded without execution when
  the probe already proves the complete postcondition).
- **Flags (D1).** `--write` is the mutation flag (`--apply` is a deprecated alias for one
  release); `--bootstrap` is removed. `--adopt-baseline <tag>` records the manifest through a
  declared baseline (v2.9.1, v2.10.1) only after every probe of that baseline holds and no later
  baseline holds; it never records beyond the baseline. `--write` refuses an empty ledger on a
  populated schema and any ledger/schema contradiction.
- **Read-only status (D2/D3).** `database status` never creates the ledger; it reports probe
  failures and a baseline suggestion, `--format json` adds `ledger_present`, `probe_failures`,
  `baseline_suggestion`, `current`; exit 2 when not current.
- **Fresh-install ledger seed (D9).** `schema.sql` carries an `INSERT` block seeding
  `core._migrations` with every manifest entry (regenerated by `infra/scripts/reset_test_db.sh`);
  a fresh install never has an empty ledger. Tests prove seed == manifest.
- **Bundled migration assets (D8).** The wheel carries `infra/migrations/manifest.yaml`, the
  manifest's SQL files and `schema.sql` as package data (`src/shared/_migrations/`, byte-parity
