...
[![Release](https://img.shields.io/badge/Release-v2.8.0-blue)](https://github.com/DariuszNewecki/CORE/releases)
...
226 rules across 54 rule documents. All 226 are mapped to enforcement engines; 0 test-quality rules are still pending mappings. "Mapped" means engine-bound — not enforced in every mode: stateless CI skips `knowledge_gate` and `llm_gate`, which need the knowledge graph and an LLM provider.
...
| G2 — Constitutional enforcement regression-tested | ⚠️ partial | Per-rule known-violating fixtures not universal |
...
>
> **Govern your own repo (BYOR):** `pip install core-runtime` and two commands bootstrap a fitted constitution. (1) `core-admin project onboard <path> --write` delivers the machinery floor (no LLM, no database). (2) `core-admin project scout <path> --write` reads your source, proposes candidate rules via LLM, and requires you to ratify each before delivery — no LLM means a curated four-rule menu instead. `core-admin code audit --offline` inside that repo enforces the ratified rules immediately. The machinery floor is bundled in the wheel — both commands work from a plain `pip install core-runtime`.
>
...
**Current Release:** v2.8.0 — Governed Delivery
