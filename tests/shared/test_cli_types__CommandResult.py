...
def test_command_result_complex_data():
    ...
    result = CommandResult(name="fix.ids", ok=True, data=complex_data)
...
def test_command_result_negative_duration():
    ...
    result = CommandResult(name="test.command", ok=True, data={}, duration_sec=-0.5)
...
def test_command_result_name_with_dots():
    ...
    result = CommandResult(
        name="module.submodule.command", ok=True, data={"status": "ok"}
    )
...
def test_command_result_logs_as_empty_list():
    ...
    result = CommandResult(name="test.command", ok=True, data={}, logs=[])
    assert result.logs == []
...
def test_command_result_multiple_log_entries():
    ...
    result = CommandResult(
        name="process.data", ok=False, data={"attempts": 3}, logs=logs
    )
