...
class TestComponentResult:
    ...
    def test_edge_case_confidence_midpoint(self):
        ...
        result = ComponentResult(component_id='test', ok=True, data={}, phase=ComponentPhase.EXECUTION, confidence=0.5)
    ...
    def test_metadata_default_factory_creates_new_dict(self):
        ...
        result2 = ComponentResult(component_id='test2', ok=False, data={}, phase=ComponentPhase.EXECUTION)
    ...
    def test_data_can_be_empty_dict(self):
        ...
        result = ComponentResult(component_id='test', ok=True, data={}, phase=ComponentPhase.EXECUTION)
        assert result.data == {}

    def test_data_can_contain_complex_structures(self):
        """Test that data can contain nested structures."""
        complex_data = {'list': [1, 2, 3], 'nested_dict': {'a': {'b': 'c'}}, 'tuple': (1, 2, 3), 'string': 'test'}
        result = ComponentResult(component_id='test', ok=True, data=complex_data, phase=ComponentPhase.EXECUTION)
        assert result.data == complex_data

    def test_next_suggested_can_be_non_empty(self):
        """Test that next_suggested can contain a component name."""
        result = ComponentResult(component_id='current', ok=True, data={}, phase=ComponentPhase.EXECUTION, next_suggested='next_component')
    ...
    def test_duration_sec_can_be_set(self):
        ...
        result = ComponentResult(component_id='test', ok=True, data={}, phase=ComponentPhase.EXECUTION, duration_sec=123.456)
        assert result.duration_sec == 123.456

    def test_ok_boolean_values(self):
        """Test that ok field accepts both True and False."""
        result_true = ComponentResult(component_id='test1', ok=True, data={}, phase=ComponentPhase.EXECUTION)
        result_false = ComponentResult(component_id='test2', ok=False, data={}, phase=ComponentPhase.EXECUTION)
