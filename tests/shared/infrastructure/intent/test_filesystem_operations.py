...
Focused on the #489 / ADR-077 §6 step 3 relaxation: the pathlib_path block
no longer asserts ``expected_match="leaf"`` for every entry. Per-entry
overrides to ``match: qualified`` are now schema-valid (used for
collision-prone leaves like ``replace`` and ``rename``). The loader still
rejects unknown match modes and missing required fields — the relaxation
is narrow.
"""
...
def test_pathlib_path_qualified_entry_now_accepted(
    intent_tree: Callable[..., Path],
) -> None:
    """#489: pathlib_path block accepts a per-entry ``match: qualified`` override.

    Pre-#489 the loader asserted every pathlib_path entry was ``leaf``;
    the relaxation lets collision-prone leaves (``replace``/``rename``)
    declare ``qualified`` to avoid str.replace and similar false
    positives.
    """
    pathlib_block = (
        "  pathlib_path:\n"
