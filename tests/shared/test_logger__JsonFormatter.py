...
def test_json_formatter_basic_log_record():
    ...
    record = logging.LogRecord(name='test_logger', level=logging.INFO, pathname='/test/path.py', lineno=42, msg='Test message %s', args=('arg1',), exc_info=None, func='test_function', sinfo=None)
...
def test_json_formatter_with_exception():
    ...
    record = logging.LogRecord(name='test_logger', level=logging.ERROR, pathname='/test/path.py', lineno=42, msg='Error occurred', args=(), exc_info=exc_info, func='test_function', sinfo=None)
    result = formatter.format(record)
    parsed = json.loads(result)
...
def test_json_formatter_custom_attributes():
    ...
    formatter = JsonFormatter()
    record = logging.LogRecord(name='test_logger', level=logging.INFO, pathname='/test/path.py', lineno=42, msg='Test message', args=(), exc_info=None, func='test_function', sinfo=None)
...
def test_json_formatter_standard_attrs_excluded():
    ...
    result = formatter.format(record)
    parsed = json.loads(result)
    standard_keys = {'name', 'msg', 'args', 'levelname', 'pathname', 'filename', 'exc_info', 'lineno', 'funcName', 'created'}
...
def test_json_formatter_different_log_levels():
    ...
    for level_name, level_num in [('DEBUG', 10), ('INFO', 20), ('WARNING', 30), ('ERROR', 40), ('CRITICAL', 50)]:
        record = logging.LogRecord(name='test_logger', level=level_num, pathname='/test/path.py', lineno=42, msg=f'{level_name} message', args=(), exc_info=None, func='test_function', sinfo=None)
        result = formatter.format(record)
        parsed = json.loads(result)
...
def test_json_formatter_valid_json_output():
    ...
    formatter = JsonFormatter()
    record = logging.LogRecord(name='test_logger', level=logging.INFO, pathname='/test/path.py', lineno=42, msg='Message with "quotes" and \n newline', args=(), exc_info=None, func='test_function', sinfo=None)
    result = formatter.format(record)
    parsed = json.loads(result)
