...
def test_symbol_id_result_initialization_with_values():
    ...
    result = SymbolIdResult(
        has_id=True,
        uuid="123e4567-e89b-12d3-a456-426614174000",
        id_tag_line_num=42,
        definition_line_num=100,
    )
...
def test_symbol_id_result_equality():
    ...
    result1 = SymbolIdResult(
        has_id=True, uuid="abc", id_tag_line_num=1, definition_line_num=2
    )
    result2 = SymbolIdResult(
        has_id=True, uuid="abc", id_tag_line_num=1, definition_line_num=2
    )
...
def test_symbol_id_result_inequality():
    ...
    result2 = SymbolIdResult(
        has_id=False, uuid="def", id_tag_line_num=3, definition_line_num=4
    )
    assert result1 != result2
...
def test_symbol_id_result_attribute_types():
    ...
    result = SymbolIdResult(
        has_id=True, uuid="test-uuid", id_tag_line_num=10, definition_line_num=20
    )
...
def test_symbol_id_result_none_uuid_and_line():
    ...
    result = SymbolIdResult(
        has_id=False, uuid=None, id_tag_line_num=None, definition_line_num=1
    )
    assert not result.has_id
    assert result.uuid is None
    assert result.id_tag_line_num is None
