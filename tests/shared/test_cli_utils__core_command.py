...
def test_core_command_dangerous_mode_dry_run():
    ...
    with patch.object(console, 'print') as mock_print:
        ...
        mock_print.assert_any_call('[bold yellow]⚠️  DRY RUN MODE[/bold yellow]\n   No changes will be made. Use [cyan]--write[/cyan] to apply.\n')
...
def test_core_command_running_inside_existing_loop():
    ...
    assert 'CORE CLI commands cannot run inside an already-running event loop' in str(exc_info.value)
...
def test_core_command_with_context_object():
    ...
    async def func_with_ctx(ctx: typer.Context):
        return f'qdrant: {ctx.obj.qdrant_service}, cognitive: {ctx.obj.cognitive_service}'
    mock_qdrant = Mock()
    mock_cognitive = Mock()
...
def test_core_command_preserves_function_metadata():
    ...
    def original_func(x: int, y: str='default') -> str:
        """Original docstring."""
        return f'{x}_{y}'
    decorated = core_command(dangerous=False, confirmation=False, requires_context=False)(original_func)
