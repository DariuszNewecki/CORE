"""Tests for CommitAuthorshipAuditWorker._audit_consequence_evidence_degraded
(ADR-148 D7, #790).

No real DB required — service_registry and ProposalSupervisionService are
mocked. Sibling to the finalization-integrity tests in
test_commit_authorship_audit_worker_finalization_integrity.py.
"""
...
async def test_audit_evidence_degraded_flags_new_violation() -> None:
    """A completed-on-reconstructed-consequence row not already open posts a
    finding and is counted as flagged."""
    ...
    proposal_svc.fetch_completed_with_degraded_consequence = AsyncMock(
    ...
    with patch("body.services.service_registry.service_registry", mock_registry):
        flagged, suppressed = await worker._audit_consequence_evidence_degraded(
            blackboard_service
        )
    ...
    assert call.kwargs["subject"] == "governance.consequence_evidence_degraded::pid-1"
...
async def test_audit_evidence_degraded_suppresses_already_open() -> None:
    """A violation whose subject is already an open finding is not re-posted."""
    worker = _make_worker_instance()

    blackboard_service = AsyncMock()
    blackboard_service.fetch_active_finding_subjects_by_prefix = AsyncMock(
        return_value={"governance.consequence_evidence_degraded::pid-1"}
    )
...
async def test_audit_evidence_degraded_no_violations() -> None:
    """No degraded-consequence rows: nothing flagged, nothing posted."""
    worker = _make_worker_instance()
    ...
    proposal_svc = AsyncMock()
    proposal_svc.fetch_completed_with_degraded_consequence = AsyncMock(return_value=[])
