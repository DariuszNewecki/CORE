...
async def test_decisions_cmd_imports_within_function():
    ...
    assert 'from shared.infrastructure.database.session_manager import get_session' in source
...
async def test_decisions_cmd_routing_logic_structure():
    ...
    assert 'await _show_recent_traces(repo, recent, agent, failures_only, details)' in source
