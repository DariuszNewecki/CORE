...
def test_load_patterns_dict_loads_single_file():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        pattern_data = {'id': 'test_category', 'patterns': ['test pattern 1', 'test pattern 2']}
...
def test_load_patterns_dict_complex_yaml_structure():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        patterns_dir = Path(tmpdir) / '.intent' / 'charter' / 'patterns'
        patterns_dir.mkdir(parents=True)
        complex_data = {'id': 'complex', 'patterns': [{'name': 'pattern1', 'regex': '\\d+'}, {'name': 'pattern2', 'regex': '[a-z]+'}], 'metadata': {'author': 'test', 'version': 1.0}}
