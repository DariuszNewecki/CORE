...
async def test_execute_with_all_metrics_within_thresholds():
    ...
    result = await evaluator.execute(operation_type='test_generation', duration_sec=30.0, memory_mb=250.0, io_operations=500)
...
async def test_execute_with_multiple_threshold_exceeded():
    ...
    assert result.metadata['has_time_issues'] == True
    assert result.metadata['has_memory_issues'] == True
    assert result.metadata['has_io_issues'] == True

@pytest.mark.asyncio
async def test_execute_with_partial_metrics():
    """Test when only some metrics are provided."""
    evaluator = PerformanceEvaluator()
    result = await evaluator.execute(operation_type='test_generation', duration_sec=70.0, memory_mb=None, io_operations=None)
    assert result.ok == False
    assert len(result.data['issues']) == 1
    assert result.data['issues'][0]['type'] == 'duration'
    assert result.data['metrics']['duration_sec'] == 70.0
    assert result.data['metrics']['memory_mb'] == None
    assert result.data['metrics']['io_operations'] == None
    assert result.data['performance_score'] < 1.0
...
async def test_execute_with_additional_kwargs_metrics():
    ...
    evaluator = PerformanceEvaluator()
    result = await evaluator.execute(operation_type='validation', duration_sec=3.0, memory_mb=80.0, io_operations=50, cpu_percent=75.0, cache_hits=100, cache_misses=20, db_queries=15)
    assert result.ok == True
...
async def test_execute_with_unknown_operation_type():
    ...
    evaluator = PerformanceEvaluator()
    result = await evaluator.execute(operation_type='unknown_operation', duration_sec=40.0, memory_mb=250.0, io_operations=600)
    assert result.ok == False
    assert len(result.data['issues']) == 3
...
async def test_calculate_score_with_all_metrics_within_thresholds():
    ...
    thresholds = {'max_duration_sec': 30, 'max_memory_mb': 300, 'max_io_operations': 500}
...
async def test_suggest_time_optimization():
    ...
    evaluator = PerformanceEvaluator()
    suggestions = {'test_generation': evaluator._suggest_time_optimization('test_generation'), 'refactor': evaluator._suggest_time_optimization('refactor'), 'sync': evaluator._suggest_time_optimization('sync'), 'validation': evaluator._suggest_time_optimization('validation'), 'query': evaluator._suggest_time_optimization('query'), 'unknown': evaluator._suggest_time_optimization('unknown')}
    assert suggestions['test_generation'] == 'Consider reducing test complexity or using batch generation'
    assert suggestions['refactor'] == 'Profile code to identify slow operations; consider incremental refactoring'
    assert suggestions['sync'] == 'Enable parallel sync operations or reduce sync scope'
    assert suggestions['validation'] == 'Cache validation results or reduce validation scope'
...
async def test_execute_includes_duration():
    ...
    evaluator = PerformanceEvaluator()
    result = await evaluator.execute(operation_type='query', duration_sec=0.5, memory_mb=30.0)
