...
async def test_check_insecure_deserialization():
    ...
    vulnerabilities = evaluator._check_insecure_deserialization(
        unsafe_code, "/test/deserialize.py"
    )
    assert len(vulnerabilities) == 1
...
async def test_execute_with_no_vulnerabilities():
    ...
    result = await evaluator.execute(
        ...
        check_scope=["secrets_exposure", "sql_injection"],
    )
    assert result.ok
...
async def test_execute_with_custom_scope():
    ...
    result = await evaluator.execute(
        ...
        code_content=code_with_secret,
        check_scope=["secrets_exposure"],
    )
    assert len(result.data["vulnerabilities"]) == 1
    assert result.data["check_scope"] == ["secrets_exposure"]
    result2 = await evaluator.execute(
        file_path="/test/scope2.py", code_content=code_with_secret, check_scope=[]
    )
...
async def test_severity_escalation_logic():
    ...
    result = await evaluator.execute(
        file_path="/test/severity.py", code_content="some clean code"
    )
    assert result.ok
    code_critical = 'api_key = "sk-test123456789012345678901234567890"'
    result_critical = await evaluator.execute(
        file_path="/test/critical.py", code_content=code_critical
    )
    assert not result_critical.ok
