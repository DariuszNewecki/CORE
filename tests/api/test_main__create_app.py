...
def test_create_app_includes_knowledge_routes():
    ...
    knowledge_route_found = any(
        route.path.startswith("/v1") and "knowledge" in str(route).lower()
        for route in app.routes
    )
    assert knowledge_route_found


def test_create_app_includes_development_routes():
    """Test that development routes are included with correct prefix and tags."""
    app = create_app()
    development_route_found = any(
        route.path.startswith("/v1") and "development" in str(route).lower()
        for route in app.routes
    )
    assert development_route_found
...
def test_create_app_route_tags():
    ...
    assert knowledge_tag_found
    assert development_tag_found
