...
def test_create_execution_plan_success(mock_dependencies):
    ...
    with patch(
        "core.prompt_pipeline.PromptPipeline.process", return_value="enriched_prompt"
    ):
...
async def test_plan_executor_create_file_success(
    mock_validate_code, mock_dependencies, tmp_path
):
    ...
    mock_validate_code.assert_called_once_with(
...
@pytest.mark.asyncio
@patch("agents.plan_executor.validate_code")
async def test_plan_executor_create_file_fails_on_validation_error(
    mock_validate_code, mock_dependencies
):
    """Sad Path: Verifies that the task fails if the code has 'error' severity violations."""
    mock_validate_code.return_value = {
        "status": "dirty",
        "violations": [
            {"rule": "E999", "message": "Syntax error!", "line": 1, "severity": "error"}
        ],
        "code": "print(",
    }
    executor = PlanExecutor(
        **{
            k: v
            for k, v in mock_dependencies.items()
            if k not in ["orchestrator_client", "generator_client", "intent_guard"]
        }
    )
...
@pytest.mark.asyncio
async def test_plan_executor_create_file_fails_if_file_exists(
...
async def test_plan_executor_edit_function_success(
    mock_code_editor_class, mock_validate_code, mock_dependencies, tmp_path
):
    ...
    params = TaskParams(
        file_path="src/feature.py", symbol_name="my_func", code=new_function_code
    )
    ...
    mock_editor_instance.replace_symbol_in_code.assert_called_once_with(
        original_code, "my_func", validated_and_formatted_snippet
    )

    executor.file_handler.add_pending_write.assert_called_once()
    executor.git_service.commit.assert_called_once_with(
        "feat: Modify function my_func in src/feature.py"
    )
