...
from mind.governance.enforcement_methods import AuditSeverity, CodePatternEnforcement
...
def test_verify_correct_detection():
    ...
    rule_data = {
        "detection": {
            ...
            "patterns": ["os.system", "subprocess.run"],
        }
    }
...
def test_verify_all_required_patterns_present():
    ...
    enforcer = CodePatternEnforcement(
        rule_id="test_rule", required_patterns=["dangerous.call", "unsafe.exec"]
    )
    context = None
    rule_data = {
        "detection": {
            "method": "ast_call_scan",
            "patterns": ["dangerous.call", "unsafe.exec", "extra.pattern"],
        }
    }
    findings = enforcer.verify(context, rule_data)
    assert len(findings) == 0
...
def test_verify_severity_passed_to_findings():
    ...
    enforcer = CodePatternEnforcement(
        rule_id="test_rule", severity=AuditSeverity.WARNING
    )
    context = None
    rule_data = {}
    findings = enforcer.verify(context, rule_data)
