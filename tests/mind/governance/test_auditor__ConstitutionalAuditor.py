...
class TestConstitutionalAuditor:
    ...
    def test_write_findings_returns_path(self):
        ...
        with patch('mind.governance.auditor.REPORTS_DIR', Path('/test/reports')), patch('mind.governance.auditor.FINDINGS_FILENAME', 'findings.json'), patch('mind.governance.auditor._repo_rel', return_value='reports/findings.json'):
            result = auditor._write_findings(findings)
            assert isinstance(result, Path)
            assert str(result) == '/test/reports/findings.json'
            mock_fs.write_runtime_json.assert_called_once_with('reports/findings.json', [{'test': 'data'}])
    ...
    def test_write_audit_evidence_returns_path(self):
        ...
        with patch('mind.governance.auditor.AUDIT_EVIDENCE_DIR', Path('/test/audit')), patch('mind.governance.auditor.AUDIT_EVIDENCE_FILENAME', 'evidence.json'), patch('mind.governance.auditor._repo_rel', side_effect=lambda x: str(x.name)), patch('mind.governance.auditor._utc_now_iso', return_value='2024-01-01T00:00:00Z'):
            result = auditor._write_audit_evidence(executed_rules=executed_rules, findings_path=findings_path, processed_findings_path=processed_findings_path, passed=True)
            assert isinstance(result, Path)
