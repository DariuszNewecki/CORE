...
def test_python_files_cached_property():
    ...
    repo_path = Path('/test/repo')
    context = AuditorContext(repo_path)
    test_py_files = [Path('/test/repo/src/main.py'), Path('/test/repo/src/utils/helper.py'), Path('/test/repo/tests/test_main.py')]
...
async def test_load_knowledge_graph_failure():
    ...
    repo_path = Path('/test/repo')
    context = AuditorContext(repo_path)
    with patch('mind.governance.audit_context.KnowledgeService') as mock_service_class, patch('mind.governance.audit_context.logger') as mock_logger:
...
def test_load_governance_resources():
    ...
    with patch('mind.governance.audit_context.get_intent_repository') as mock_get_repo, patch('mind.governance.audit_context.logger') as mock_logger:
