...
class TestValidationReport:
    ...
    def test_initialization_with_custom_values(self):
        ...
        warnings = ['warning1']
        report = ValidationReport(valid=False, errors=errors, warnings=warnings, documents_checked=5, documents_valid=3, documents_invalid=2)
    ...
    def test_equality_comparison(self):
        ...
        report1 = ValidationReport(valid=True, errors=['test_error'], warnings=[], documents_checked=1, documents_valid=1, documents_invalid=0)
        report2 = ValidationReport(valid=True, errors=['test_error'], warnings=[], documents_checked=1, documents_valid=1, documents_invalid=0)
    ...
    def test_string_representation(self):
        ...
        report = ValidationReport(valid=False, errors=['Error 1', 'Error 2'], warnings=['Warning 1'], documents_checked=10, documents_valid=7, documents_invalid=3)
        str_rep = str(report)
    ...
    def test_empty_report_is_valid(self):
        ...
        report = ValidationReport(valid=True, errors=[], warnings=['Some warning'], documents_checked=5, documents_valid=5, documents_invalid=0)
    ...
    def test_invalid_report_has_errors(self):
        ...
        report1 = ValidationReport(valid=False, errors=['Validation failed'], warnings=[], documents_checked=5, documents_valid=5, documents_invalid=0)
        report2 = ValidationReport(valid=False, errors=[], warnings=[], documents_checked=5, documents_valid=3, documents_invalid=2)
        assert report1.valid == False
        assert len(report1.errors) > 0
        assert report2.valid == False
    ...
    def test_documents_consistency(self):
        ...
        report = ValidationReport(valid=True, errors=[], warnings=[], documents_checked=10, documents_valid=8, documents_invalid=2)
        assert report.documents_checked == report.documents_valid + report.documents_invalid
