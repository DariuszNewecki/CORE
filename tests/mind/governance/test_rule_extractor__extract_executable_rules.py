...
def test_extract_executable_rules_basic_extraction():
    ...
    policies = {
        "policy1": {
            "rules": [
                {
                    "id": "rule1",
                    "statement": "Files must have proper headers",
    ...
    loader = TestEnforcementMappingLoader(
        {
            "rule1": {
                "engine": "regex_checker",
                "params": {"pattern": "^# Copyright"},
                "scope": {"applies_to": ["src/**/*.py"], "excludes": ["tests/"]},
            }
        }
    )
...
def test_extract_executable_rules_multiple_rules():
    ...
    loader = TestEnforcementMappingLoader(
        {
            "rule1": {"engine": "engine1", "params": {}},
            "rule2": {"engine": "engine2", "params": {}},
        }
    )
    result = extract_executable_rules(policies, loader)
    assert len(result) == 2
...
def test_extract_executable_rules_invalid_policy_structure():
    ...
    policies = {
        "policy1": "not a dict",
        "policy2": {"rules": "not a list"},
        "policy3": {"rules": [{"id": "rule1"}]},
        "policy4": {
            "rules": [
                {
                    "id": "rule4",
...
def test_extract_executable_rules_scope_fallback():
    ...
    loader = TestEnforcementMappingLoader(
        {"rule1": {"engine": "engine1", "params": {}, "scope": "invalid"}}
    )
    result = extract_executable_rules(policies, loader)
    assert len(result) == 1
    assert result[0].scope == ["src/**/*.py"]
