...
import pytest
from mind.governance.enforcement_methods import KnowledgeSSOTEnforcement
from mind.governance.enforcement_methods import AuditSeverity
...
async def test_verify_async_table_empty():
    ...
    with patch('shared.infrastructure.database.session_manager.get_session') as mock_get_session:
...
async def test_verify_async_pk_uniqueness_check_fails():
    ...
    mock_count_result = MagicMock()
    mock_count_result.scalar_one.return_value = 5
    mock_session.execute.side_effect = [mock_count_result, Exception('Syntax error in PK query')]
    with patch('shared.infrastructure.database.session_manager.get_session') as mock_get_session:
        mock_get_session.return_value.__aenter__.return_value = mock_session
        findings = await enforcement.verify_async(context, rule_data)
        assert len(findings) > 0
        assert any(('PK uniqueness check failed' in finding.message for finding in findings))
        assert all((finding.severity == AuditSeverity.ERROR for finding in findings))
...
async def test_verify_async_multiple_tables_mixed_results():
    ...
    mock_session.execute.side_effect = [mock_result1, mock_dup_result3, mock_result2, mock_result3, mock_dup_result3, mock_result4]
    with patch('shared.infrastructure.database.session_manager.get_session') as mock_get_session:
        mock_get_session.return_value.__aenter__.return_value = mock_session
        findings = await enforcement.verify_async(context, rule_data)
...
async def test_init_with_custom_severity():
    ...
    enforcement = KnowledgeSSOTEnforcement(rule_id='custom.rule', severity=AuditSeverity.WARNING)
