...
def test_no_downgrade_wrong_rule_id():
    ...
    findings = [
        {
            "rule_id": "other_rule",
            "symbol_key": "module.function",
            "severity": "error",
            "message": "test finding",
        }
    ]
    symbol_index = {
        "module.function": {"entry_point_type": "cli", "pattern_name": "main"}
    }
    result = apply_entry_point_downgrade_and_report(
        findings=findings, symbol_index=symbol_index, write_reports=False
    )
...
def test_write_reports_without_file_handler_raises():
    ...
    with pytest.raises(ValueError, match="write_reports=True requires file_handler"):
        apply_entry_point_downgrade_and_report(
            findings=findings, symbol_index=symbol_index, write_reports=True
        )
