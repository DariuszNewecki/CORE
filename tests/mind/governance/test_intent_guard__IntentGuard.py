...
class TestIntentGuard:
    ...
    def test_check_no_write_intent_allowed(self, tmp_path):
        ...
        violation = guard._check_no_write_intent((repo_root / test_path).resolve(), test_path)
    ...
    def test_matches_pattern_glob_matching(self, tmp_path):
        ...
        repo_root.mkdir()
        guard = IntentGuard(repo_root)
        assert guard._matches_pattern('src/main.py', 'src/main.py') == True
        assert guard._matches_pattern('src/main.py', 'src/*.py') == True
        assert guard._matches_pattern('src/utils/helper.py', 'src/**/*.py') == True
        assert guard._matches_pattern('src/main.py', 'tests/*.py') == False
        assert guard._matches_pattern('src/main.py', '') == False
        assert guard._matches_pattern('docs/api/index.md', 'docs/**/*.md') == True
    ...
    def test_is_emergency_mode_detection(self, tmp_path):
        ...
        repo_root.mkdir()
        guard = IntentGuard(repo_root)
        assert guard._is_emergency_mode() == False
        lock_path = repo_root / '.intent' / 'mind' / '.emergency_override'
        lock_path.parent.mkdir(parents=True)
        lock_path.touch()
        assert guard._is_emergency_mode() == True
    ...
    def test_apply_rule_action_deny(self, tmp_path):
        ...
        deny_rule = PolicyRule(name='test_deny', pattern='*.tmp', action='deny', description='Temporary files not allowed', severity='error', source_policy='test_policy')
        violations = guard._apply_rule_action(deny_rule, (repo_root / 'temp.tmp').resolve(), 'temp.tmp')
    ...
    def test_apply_rule_action_warn(self, tmp_path, caplog):
        ...
        guard = IntentGuard(repo_root)
        from mind.governance.intent_guard import PolicyRule
        warn_rule = PolicyRule(name='test_warn', pattern='*.log', action='warn', description='Log files should be rotated', severity='warning', source_policy='test_policy')
        violations = guard._apply_rule_action(warn_rule, (repo_root / 'app.log').resolve(), 'app.log')
        assert violations == []
    ...
    def test_check_policy_rules_integration(self, tmp_path):
        ...
        violations = guard._check_policy_rules((repo_root / test_path).resolve(), test_path)
        assert isinstance(violations, list)
