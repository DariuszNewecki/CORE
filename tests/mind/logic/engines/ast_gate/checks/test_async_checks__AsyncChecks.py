...
class TestCheckRestrictedEventLoopCreation:
    ...
    def test_direct_run_until_complete_detected(self):
        ...
        tree = ast.parse(code)
        result = AsyncChecks.check_restricted_event_loop_creation(tree, ['asyncio.new_event_loop'])
        assert len(result) == 1
...
class TestCheckNoImportTimeAsyncSingletons:
    ...
    def test_module_level_disallowed_call_detected(self):
        ...
        tree = ast.parse(code)
        result = AsyncChecks.check_no_import_time_async_singletons(tree, ['aiohttp.TCPConnector'])
        assert len(result) == 1
