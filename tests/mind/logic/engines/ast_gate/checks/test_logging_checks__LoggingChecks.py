...
"""
    ...
    assert result == [f"Line {expected_line}: Replace print() with logger.info() or logger.debug()"]
...
"""
    ...
    assert len(result) == 3
    assert all("Replace print() with logger.info() or logger.debug()" in msg for msg in result)
