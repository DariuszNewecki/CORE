...
async def test_canary_passed_true():
    ...
    result = await check.verify(
        file_path=Path("/full/path/to/file.txt"),
        params={"canary_passed": True}
    )
    assert result == []
...
async def test_canary_passed_false():
    ...
    result = await check.verify(
        file_path=Path("/full/path/to/file.txt"),
        params={"canary_passed": False}
    )
    assert result == ["Canary audit required: Operation must pass in staging/isolation first."]
