...
from mind.logic.engines.cli_gate.models import AuditFinding, AuditSeverity, CliCheck
...
class TestNoLayerExposureCheck:
    ...
    def test_verify_no_forbidden_in_params(self, check_instance: NoLayerExposureCheck) -> None:
    ...
    def test_verify_forbidden_resources_none(self, check_instance: NoLayerExposureCheck) -> None:
    ...
    def test_verify_forbidden_resources_empty_list(self, check_instance: NoLayerExposureCheck) -> None:
        """Edge case: forbidden_resources empty list."""
        commands: list[dict[str, Any]] = [{"name": "layer.resource.command"}]
        result = check_instance.verify(commands, {"forbidden_resources": []})
        assert result == []

    def test_verify_command_with_no_name(self, check_instance: NoLayerExposureCheck) -> None:
        ...
        result = check_instance.verify(commands, {"forbidden_resources": ["forbidden_layer"]})
        assert result == []

    def test_verify_happy_path_no_exposure(self, check_instance: NoLayerExposureCheck) -> None:
        """Happy path: no command exposes a forbidden resource."""
        commands: list[dict[str, Any]] = [
            {"name": "allowed.resource.do_thing"},
            {"name": "safe.layer.run"},
        ]
        result = check_instance.verify(commands, {"forbidden_resources": ["forbidden_layer", "restricted"]})
        assert result == []
    ...
    def test_verify_multiple_exposures(self, check_instance: NoLayerExposureCheck) -> None:
    ...
    def test_verify_multiple_commands_same_resource(self, check_instance: NoLayerExposureCheck) -> None:
    ...
    def test_verify_resource_first_segment(self, check_instance: NoLayerExposureCheck) -> None:
        """Verify only the first segment before '.' is checked as resource."""
        commands: list[dict[str, Any]] = [
            {"name": "good.restricted.cmd"},
        ]
        result = check_instance.verify(commands, {"forbidden_resources": ["restricted"]})
        assert result == []

    def test_verify_case_sensitivity(self, check_instance: NoLayerExposureCheck) -> None:
    ...
    def test_verify_context_contains_sorted_forbidden(self, check_instance: NoLayerExposureCheck) -> None:
