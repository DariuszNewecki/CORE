...
async def test_match_basic_glob():
    ...
    engine = GlobGateEngine()
    assert engine._match('src/file.py', '*.py') == True
    assert engine._match('src/file.py', '*.js') == False
    assert engine._match('src/utils/file.py', 'src/*') == True
    assert engine._match('src/utils/file.py', 'utils/*') == False
