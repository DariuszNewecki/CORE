...
class CognitiveService:
    ...
    def __init__(self, repo_path: Path):
        ...
        log.info(f"CognitiveService initialized with {len(self._roles_map)} roles and {len(self._resources_map)} resources.")
    ...
    def get_client_for_role(self, role_name: str) -> BaseLLMClient:
        ...
        if not role_config:
            raise ValueError(f"Cognitive role '{role_name}' is not defined in the constitution.")

        resource_name = role_config.get('assigned_resource')
        resource_config = self._resources_map.get(resource_name)
        if not resource_config:
            raise ValueError(f"Resource '{resource_name}' for role '{role_name}' is not in the manifest.")
        ...
        if not all([api_url, api_key, model_name]):
            raise ValueError(f"Configuration for resource prefix '{env_prefix}' is missing in the environment.")

        client = BaseLLMClient(api_url=api_url, api_key=api_key, model_name=model_name)
        self._client_cache[role_name] = client

        log.info(f"Instantiated LLM client for role '{role_name}' using resource '{resource_name}' ({model_name}).")
        return client