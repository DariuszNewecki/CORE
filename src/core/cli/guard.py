...
from src.system.guard.drift_detector import (
    load_manifest,
...
def _handle_drift(args: argparse.Namespace) -> int:
    ...
    code_caps = collect_code_capabilities(
        ...
        require_kgb=args.strict_intent,
    )
    ...
    print(json.dumps(report_dict, indent=2) if args.format == "json" else "")  # table already printed
...
def register_guard_command(subparsers: argparse._SubParsersAction) -> None:
    ...
    drift.add_argument("--strict-intent", action="store_true",
                       help="Require KnowledgeGraphBuilder; disallow regex fallback")
