...
class CognitiveService:
    ...
    def __init__(self, repo_path: Path, qdrant_service: QdrantService | None = None):
        ...
        # Cache for non-secret config values to avoid repeated DB lookups
    ...
    async def initialize(self) -> None:
        """Load resources, roles, and config cache from DB."""
        async with self._init_lock:
            ...
            try:
                ...
                async with get_session() as session:
                    # Load config into a temporary service to extract the cache
                    temp_config_service = await ConfigService.create(session)
                    self._config_cache = temp_config_service._cache
    ...
    async def _create_provider_for_resource(self, resource: LlmResource) -> AIProvider:
        ...
        # Create a fresh session and ConfigService for this operation
        # This prevents holding onto stale sessions (which causes connection leaks)
        async with get_session() as session:
            # Reconstruct ConfigService with the new session and existing cache
            config_service = ConfigService(session, self._config_cache)
            ...
            try:
                api_key = await config_service.get_secret(
                    f"{prefix}_API_KEY",
                    audit_context=f"cognitive_service:{resource.name}",
                )
    ...
    async def aget_client_for_role(self, role_name: str) -> LLMClient:
        ...
        try:
            ...
            # Create a temporary config service for the LLMResourceConfig helper
            # Note: This session is only used for initialization logic inside create()
            async with get_session() as session:
                config_service = ConfigService(session, self._config_cache)
