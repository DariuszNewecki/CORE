...
CRITICAL BOUNDARY ENFORCEMENT:
    .intent/ directory access is RESTRICTED to IntentRepository only.
    PathResolver exposes ONLY intent_root - no subdirectory access.
    All .intent/ navigation must go through IntentRepository APIs.
"""
...
class PathResolver:
    ...
    CONSTITUTIONAL BOUNDARY:
        - .intent/ access limited to intent_root property only
        - All .intent/ subdirectory navigation via IntentRepository
    """
    ...
    def intent_root(self) -> Path:
        """
        Root of .intent/ directory - RESTRICTED ACCESS.

        CONSTITUTIONAL BOUNDARY:
        This property exists ONLY for IntentRepository to use.
        All other code must use IntentRepository APIs to access .intent/ contents.

        Do NOT use this to construct paths like:
            settings.paths.intent_root / "policies" / "something.json"  # WRONG

        Instead use:
            get_intent_repository().load_policy("policies/something")  # CORRECT
        """
    ...
    # =========================================================================
    # PROMPTS (runtime artefacts in var/)
    # =========================================================================
    ...
    def validate_structure(self) -> list[str]:
        ...
        # .intent is managed by governance tooling; do not create it here.
        if not self.intent_root.exists():
            errors.append(f"Missing required directory: .intent/ at {self.intent_root}")
    ...
    def policy(self, policy_id: str) -> Path:
        """
        LEGACY: Resolve a policy file by id/name.

        WARNING:
            This is a compatibility shim for older code (especially governance checks)
            that still does `settings.paths.policy("...")`.

            This method navigates under `.intent/policies/`, which is otherwise a
            restricted boundary. Prefer IntentRepository APIs for all new code.

        Args:
            policy_id: Policy stem (e.g., "atomic_actions") or relative path under
                      policies (e.g., "architecture/atomic_actions").

        Returns:
            Path to a matching policy file (.json/.yaml/.yml).

        Raises:
            FileNotFoundError: if policy cannot be resolved.
        """
        logger.warning(
            "LEGACY SHIM USED: settings.paths.policy(%r). Prefer IntentRepository APIs.",
            policy_id,
        )

        policies_root = self.intent_root / "policies"
        if not policies_root.exists():
            raise FileNotFoundError(f"Policies root not found: {policies_root}")

        raw = policy_id.strip().replace("\\", "/")
        raw = raw.removeprefix("policies/").removeprefix(".intent/policies/")

        # If caller passed a relative path, try direct resolution first (with or without extension)
        ...
        # Fallback: treat as stem and search anywhere under policies/
        patterns = [
            str(policies_root / "**" / f"{Path(raw).name}.json"),
            str(policies_root / "**" / f"{Path(raw).name}.yaml"),
            str(policies_root / "**" / f"{Path(raw).name}.yml"),
        ]
