...
def _parse_block(
    *,
    ...
    leaf, but collision-prone attribute names like ``replace``/``rename``/
    ``open`` are declared qualified to avoid firing on ``str.replace``,
    ``tarfile.open(mode="w:gz")``, and similar — see #489 / ADR-077 §6
    step 3 phased re-plan).
    """
