...
from system.guard.drift_detector import (
    ...
    load_manifest,  # capability-only view
    write_report,
)
...
def _ux_defaults(root: Path, explicit: Optional[Path]) -> Dict[str, Any]:
    ...
    ux = (((raw.get("operator_experience") or {})
           .get("guard") or {})
           .get("drift") or {})
    # Fallbacks if not defined in manifest
    return {
...
def register(app: typer.Typer) -> None:
    ...
    def drift(
        ...
        # Read UX defaults from raw manifest
        ...
        # Load capability mapping from .intent
        ...
        # Evidence JSON (path relative to repo root by default)
        if ux["evidence_json"]:
        ...
        if fmt == "json":
            ...
            typer.echo(json.dumps(report_dict, indent=2))  # fallback
