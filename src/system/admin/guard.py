...
from system.guard.capability_discovery import collect_code_capabilities, load_manifest_capabilities
# --- END OF FIX ---
...
def _ux_defaults(root: Path, explicit: Optional[Path]) -> Dict[str, Any]:
    ...
    return {
        ...
        "labels": ux.get("labels", {
            "none": "NONE", "success": "✅ No capability drift", "failure": "🚨 Drift detected"
        }),
    }
...
def _print_table(report_dict: dict, labels: Dict[str, str]) -> None:
    ...
    if not mismatches:
        ...
        lines = [f"- {m.get('capability')}: manifest(...) != code(...)" for m in mismatches]
        table.add_row("Mismatched mappings", "[yellow]" + "\n".join(lines) + "[/yellow]")

    status = f"[bold green]{labels['success']}[/bold green]" if _is_clean(report_dict) else f"[bold red]{labels['failure']}[/bold red]"
...
def register(app: typer.Typer) -> None:
    ...
    def drift(
        ...
        code_caps = collect_code_capabilities(root, include_globs=include, exclude_globs=exclude, require_kgb=strict)
    ...
    def kg_export(
        ...
        caps = collect_code_capabilities(root, include_globs=include, exclude_globs=exclude, require_kgb=require_kgb)
