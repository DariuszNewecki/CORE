...
def register(app: typer.Typer) -> None:
    ...
    def proposals_list() -> None:
        ...
        approvers_config = load_yaml_file(
            settings.MIND / "constitution" / "approvers.yaml"
        )
        ...
        for prop_path in proposals:
            ...
            status = (
                "✅ Ready" if current >= required else f"⏳ {current}/{required} sigs"
            )

            log.info(f"\n  - **{prop_path.name}**: {justification.strip()}")
            log.info(f"    Target: {config.get('target_path')}")
            log.info(
                f"    Status: {status} ({'Critical' if is_critical else 'Standard'})"
            )
    ...
    def proposals_approve(
        ...
        approver_keys = {
            ...
            for a in approvers_config.get("approvers", [])
        }
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if success:
                ...
                live_target_path.write_text(
                    proposal.get("content", ""), encoding="utf-8"
                )
