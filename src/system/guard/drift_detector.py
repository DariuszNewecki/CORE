...
try:
    ...
    yaml = None  # We'll give a helpful error in load_manifest()

_CAPABILITY_RE = re.compile(r"^\s*#\s*CAPABILITY:\s*([A-Za-z0-9_.\-:/]+)(.*)$")
_INLINE_KV_RE = re.compile(r"\[\s*([^\]]+)\s*\]")
_KV_PAIR_RE = re.compile(r"([A-Za-z0-9_.\-:/]+)\s*=\s*([^\s,;]+)")
...
def _try_import_kgb():
    ...
    try:
        mod = importlib.import_module("system.tools.codegraph_builder")  # runtime-only
...
def _find_manifest(start: Path) -> Path:
    ...
    candidates = [
        start / ".intent" / "project_manifest.yaml",
        start / ".intent" / "manifest.yaml",
    ]
    for p in candidates:
        if p.exists():
            return p
    raise FileNotFoundError(
        "No manifest found in .intent/ (looked for project_manifest.yaml, manifest.yaml)"
    )
...
def _normalize_manifest_caps(raw: dict) -> Dict[str, CapabilityMeta]:
    ...
    def extract_from_node(node: Any) -> Optional[Dict[str, CapabilityMeta]]:
        ...
        for key in ("capabilities", "required_capabilities", "expected_capabilities",
                    "capability_map", "capability_registry"):
            if key in node:
        ...
        # Nested component lists that hold capabilities inside
...
# -------------------------
# Knowledge graph artifact reader (strict mode helper)
# -------------------------
def _extract_cap_meta_from_node(node: Dict[str, Any]) -> Optional[CapabilityMeta]:
    ...
    # Tag list like ["CAPABILITY: foo", ...]
    if cap is None:
        tags = node.get("tags") or node.get("labels")
        if isinstance(tags, list):
            for t in tags:
                if isinstance(t, str):
                    m = re.match(r"^\s*CAPABILITY:\s*([A-Za-z0-9_.\-:/]+)\s*$", t)
                    if m:
                        cap = m.group(1)
                        break
...
def _collect_from_kgb_artifact(root: Path) -> Dict[str, CapabilityMeta]:
    ...
    """
    candidates = [
        root / "reports" / "knowledge_graph.json",
        root / "reports" / "codegraph.json",
        root / "reports" / "knowledge_graph.ndjson",
        root / "artifacts" / "knowledge_graph.json",
        root / ".intent" / "knowledge_graph.json",
    ]
...
def _collect_from_kgb(root: Path) -> Dict[str, CapabilityMeta]:
    ...
    try:
        ...
        graph = builder.build()  # type: ignore[attr-defined]
        caps: Dict[str, CapabilityMeta] = {}
        # networkx-like
        if hasattr(graph, "nodes"):
            try:
                for node in getattr(graph, "nodes"):
                    attrs = {}
                    try:
                        attrs = graph.nodes[node]  # type: ignore[index]
                    except Exception:
                        pass
...
def collect_code_capabilities(
    ...
    # 2) Try on-disk artifact
    artifact_caps = _collect_from_kgb_artifact(root)
    if artifact_caps:
        return artifact_caps

    # 3) Strict mode: fail now (no guessing via grep)
    if require_kgb:
        raise RuntimeError(
            "Strict intent mode: No capabilities found from KnowledgeGraphBuilder or artifacts. "
            "Run `core-admin guard kg-export` first (or rerun without --strict-intent)."
        )

    # 4) Non-strict fallback: grep
...
def detect_capability_drift(
    manifest_caps: Dict[str, CapabilityMeta],
