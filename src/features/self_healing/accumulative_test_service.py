...
class AccumulativeTestService:
    ...
    async def accumulate_tests_for_file(
        ...
        if alignment.get("status") == "failed":
            logger.error(
                "❌ Police Agent failed to align %s. Aborting test generation.",
                file_path,
            )
            return {
                "file": file_path,
        ...
        for i, symbol in enumerate(symbols, 1):
            ...
            if result["status"] == "success" and result["passed"]:
                ...
                logger.debug("✓ Test generated and passed for %s", symbol)
            else:
                failed_symbols.append(symbol)
                logger.warning(
                    "✗ Failed to generate test for %s: %s", symbol, result.get("reason")
                )
            ...
            if write:
                ...
                try:
                    await asyncio.to_thread(
                        run_poetry_command,
                        f"🧹 Cleaning test imports for {test_file_path.name}",
                        [
                            "ruff",
                            "check",
                            "--fix",
                ...
                logger.info("✅ Tests accumulated and cleaned: %s", test_file_path)
            else:
                logger.info(
                    "💧 [DRY RUN] Would write %d tests for %s",
