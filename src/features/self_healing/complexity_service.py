...
def _propose_constitutional_amendment(proposal_plan: dict[str, Any]):
    ...
    logger.info(
        "Constitutional amendment proposed at: %s", proposal_path.relative_to(REPO_ROOT)
    )
    return True
...
async def _run_capability_reconciliation(
    ...
    try:
        ...
        logger.info("AI Constitutionalist provided a valid reconciliation plan.")
        return reconciliation_result
    except (json.JSONDecodeError, ValueError) as e:
        logger.error("Failed to parse reconciliation plan from AI: %s", e)
...
async def _async_complexity_outliers(
    ...
    for file_rel_path in outlier_files:
        try:
            ...
            if dry_run:
                logger.info("Refactoring Plan for %s:", file_rel_path)
            ...
            logger.error("Failed to process '%s': %s", file_rel_path, e, exc_info=True)
            continue
