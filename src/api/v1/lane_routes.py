...
runs no ActionExecutor work and no direct database access. Per the settled
gate-location decision (#652) the validation gate runs decoupled — the agent
dispatches `assisted.validate_diff` through the general action-run surface
(`POST /fix/run/{fix_id}`), never inside a lane handler. `propose` is
transport-only: privileged re-verification of the named validation run
against `core.fix_runs` (ADR-154 D2) happens in the Body-owned trusted
validation service, reached via LaneService — never in this route.

CONSTITUTIONAL:
- No business logic and no Body bypass: the route routes through LaneService
  (Will), mirroring proposals_routes → ProposalService.
- No direct database access anywhere in this module (ADR-154 D2 moved the
  former `get_api_session` verdict re-read into Body).
"""
...
class ProposeRequest(BaseModel):
    ...
    via `POST /fix/run/{fix_id}`, tracked in `core.fix_runs`). LaneService
    (via the Body-owned trusted validation service) re-reads that run's
    persisted verdict — it never trusts the caller's word that validation
    passed.
    """
...
@router.post(
    ...
    description=(
        ...
        "This endpoint is transport-only: LaneService constructs a frozen "
        "`ValidatedRemediationCandidate` via the Body-owned trusted validation "
        "service (ADR-154 D2), which re-reads the persisted verdict of the "
        "named validation run from `core.fix_runs` (it never trusts the "
        "caller's claim that validation passed) and confirms the submitted "
        "patch matches the bytes that were validated. The existing "
