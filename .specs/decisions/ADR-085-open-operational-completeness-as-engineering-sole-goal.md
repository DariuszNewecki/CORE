...
| 2026-06-02 | F-40 OEM API surface | ✅ `status: shipping`. All three clauses of the exit criterion satisfied in a single session: (1) registry row flipped in `CORE-Features.md`; (2) **documented public contract** = `papers/CORE-OEM-API.md` (route classification, F-40.1) + ADR-087 (versioning + stability policy, F-40.2) + `.specs/contracts/oem_api_v1.openapi.json` (machine-readable spec, F-40.3, 48 paths, 49 operations); (3) **sidecar-shape commercial features F-20/F-34/F-45 can attach without private hooks** — recorded by F-40.4's design-walk in `.specs/verification/F-40-sidecar-walk.md` (F-20: 6 endpoints, 0 gaps; F-34: ~35 endpoints, 0 gaps; F-45 read-side: 5 endpoints, 0 gaps). F-47 is **explicitly NOT a F-40 dependent** (not a FastAPI consumer; its "API" is the Qdrant wire protocol) — the ADR-084 D8 sidecar bucket assignment stays correct as managed-infrastructure but the F-40-attachment edge is null for F-47. Closed via #414 (parent) + #550/#551/#552/#553 (F-40.1–F-40.4 MVP sub-issues). F-40.5 (#554 — auth) and F-40.6 (#555 — host binding + rate limiting) remain open as Phase B post-exit work per ADR-087 D8 + ADR-085 5+3 row's literal sidecar-attachability scope. |
| 2026-06-03 | F-27 Local LLM | ✅ `status: shipping`. **Criterion amended by ADR-089 D1** (per ADR-085 D7 on-equal-footing amendment clause): the original "≥7-day reliable Solo run" usage window is replaced with a one-shot capability demonstration. The original row in §Context above stays verbatim per ADR-074 D13 + ADR-080 §D5 append-only precedent. **Demonstration executed 2026-06-03 18:20:23Z–18:28:27Z**: routing flipped via `core.role_resource_assignments` so all 10 generative cognitive roles routed only to local Ollama (`ollama_qwen_coder_small` qwen2.5-coder:3b for Architect/Coder/CodeReviewer/LocalCoder/LocalReasoner/Planner/RemoteCoder; `ollama_reasoner` qwen2.5:7b for CapabilityTagger/ConstitutionalCoherenceAnalyst/DocstringWriter) per ADR-024 D1; Vectorizer untouched per ADR-089 D4 (embedding out of scope). `core_context.action_executor.execute("build.tests", source_file="src/shared/models/audit_rendering.py", write=False)` returned `ok=True` in 90.36s (1186 prompt + 1150 completion tokens via qwen2.5-coder:3b under RemoteCoder role). `core.llm_exchange_log` window query: **0 remote-locality generative calls** during the window; 1 local generative call (qwen-3b); 3 local embedding calls (nomic, expected). Routing restored symmetrically immediately after demonstration (UPDATE counts 10/13/2 inverse of flip-direction 13/10/2 — verified by SELECT showing baseline state). Closed via #401. `CORE-Features.md` registry row flipped `partial` → `shipping`; total now 31 shipping / 0 partial / 17 roadmap. |
| 2026-06-06 | F-41 + F-42 + F-43 Extension interfaces | ✅ all three `status: shipping`; the trio-level "one first-party non-code instantiation" clause is satisfied by F-41's `intent_yaml` + `spec_markdown` artifact_type declarations + F-42's sensors observing them (`coherence_sensor` → `spec_markdown`; `meta_validator` → `intent_yaml`). **F-41 shipped 2026-06-05** — all 9 ADR-090 §Verification gates closed; key closing commits `e3186d98` (gate 3, crawl_scopes retired), `0854243e` (gate 4, Python pipeline end-to-end via ADR-091 D5 Phase 3), `f08ca3a1` (gate 6, CCC migration via ADR-091 D5 Phase 4), `9db87bdf` (gate 9, advisory anti-regression rule; engine check at #566). Closed via #415. **F-42 shipped 2026-06-06** — ADR-091 D5 fully implemented; closing commits `ab0a5706` (engines + governance sensor closing #575), `416f5fdb` (promotion to blocking), `c9953d46` (Phase 7 sensor-supported-by-declaration promotion + self-detected asymmetry fix). Published sensor contract structurally stable: single `post_finding` API + `resolution_mechanism` field + sensor-to-artifact_type coherence rule + dispatch sensor model. Closed via #416. **F-43 shipped 2026-06-06** — **criterion amended by ADR-092 D1** (per ADR-085 D7 on-equal-footing amendment clause): the original "one first-party non-code instantiation" is replaced with action-layer registry-coupling refusal demonstration. The original row in §Context above stays verbatim per ADR-074 D13 + ADR-080 §D5 append-only precedent. **Demonstration executed 2026-06-06**: 17 atomic actions in `src/body/atomic/` declare their target artifact_type via `operational_capabilities.yaml` augment per ADR-092 D4 Option B (16 `fix.*` → `[python]`, `build.tests` → `[test]`, `fix.format` → `[python, test]` exercising the multi-type case). `ActionExecutor.execute()` refuses dispatch with `ActionResult(ok=False, data={"error": "Action declared unregistered artifact_type", "constitutional_basis": "ADR-091 D6 item 3 registry-coupling enforcement; ADR-092 D1 F-43 exit criterion"})` when an action declares an unregistered artifact_type — verified end-to-end against real ActionExecutor (smoke captured WARNING from `body.atomic.executor` confirming chokepoint short-circuit before stub action's `_trap_executor` could run). Negative-path test `tests/body/atomic/test_executor_artifact_type_refusal.py` is pytest-runnable per ADR-092 D1's "happy-path-only is not accepted as evidence" clause. ADR-091 D6's four-item forward contract resolves: items 1+3+4 ship under F-43; item 2 (`governance.taxonomy.action_supported_by_declaration` coherence rule) parked per ADR-092 D2 with D5 unparking trigger when a second non-Python action ships. Closed via #417. `CORE-Features.md` registry rows flipped for all three (latent F-41+F-42 flips reconciled in the F-43 ship-close commit per ADR-085 §4 step 3); total now 34 shipping / 0 partial / 14 roadmap. |

Remaining 5+3 items: three quality goals only (docs polish #561, demo reliability #562, signal quality #563). **All five feature commitments closed.** Constraint in D1 not yet relaxed — relaxation requires governor act per D5 once the three quality goals are satisfied.
