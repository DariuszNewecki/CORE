...
**Registry-side coverage gap (filed for follow-up):** `.specs/phases/*.yaml` is currently un-typed — neither `spec_markdown` (`*.md`) nor `intent_yaml` (`.intent/**`) covers it. Resolution choices (extend `intent_yaml` glob OR declare a new `phase_yaml` artifact_type) are filed as a separate sub-issue. CCC's `_phase_paths` discovery surface (touched in Phase 4) is parked until the gap closes.
...
- **Phase 4 — CCC sub-discovery routes through registry.** `CoherenceChecker` runs inline during the audit cycle, not as a Worker (per D3 amendment). Its discovery surfaces — `_adr_paths`, `_northstar_paths`, `_phase_paths` (subject to D3 gap resolution), and the row-check discovery in `mind.coherence.checks.{row2_grounding, row3_citation, row4_naming}` — consult `IntentRepository.get_artifact_type("spec_markdown")` for ADR/paper/northstar discovery. Row checks that span `.intent/` YAML (e.g. row4 naming consistency) additionally consult `get_artifact_type("intent_yaml")`. CCC findings retain their existing subject format; D2's canonical subject format applies to Worker-emitted findings only, so a separate CCC subject migration is out of scope here and tracked separately if pursued. Verification: CCC outcome (finding identifier set) before vs after migration is identical. Closes F-41 verification gate 6 (fully).
...
6. **CCC sub-discovery migrated.** `CoherenceChecker._adr_paths`, `_northstar_paths`, and the row-check discovery surfaces in `mind.coherence.checks.{row2_grounding, row3_citation, row4_naming}` consult the spec_markdown (and intent_yaml where applicable) registry discovery globs. CCC runs inline during audit (not as a Worker per D3) — its subject format is unchanged by this ADR. CCC outcome (finding identifier set) under registry-routed discovery identical to pre-migration outcome. F-41 verification gate 6 fully met. (`_phase_paths` deferred per D3 gap.)
