...
7. **Land `.intent/enforcement/config/audit_ingest.yaml`** with `quality_ingest_cap: 25` and an empty `enabled_rules:` list. Loader code in `audit_ingest_worker` reads the cap and exposes a helper. **No sensor wiring yet** — this is the constraint substrate D5 specifies, ready for the future sensor hookup. The worker is declared at `.intent/workers/quality_ingest_worker.yaml`.
