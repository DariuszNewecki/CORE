...
- A governance rule encoding this principle is not yet authored. Candidate statement: *"Only files under `src/**/workers/**` (or the Worker base class itself) may contain INSERT statements against `core.blackboard_entries`. Service-layer INSERTs into the attribution table are forbidden."* Enforcement via an AST/string sensor on `src/**/*.py` is straightforward. Mapping and rule authoring are governor-domain work.
  ...
  **Closed (Open Debt status as of 2026-05-26):** Rule `architecture.blackboard.worker_only_inserts` was subsequently authored (enforcement: blocking, authority: policy, phase: audit). The implemented scope is tighter than the candidate statement above — it requires INSERTs to originate from the **Worker base class** at `src/shared/workers/base.py` specifically, rather than any file under `src/**/workers/**`, because only the base class's `_post_entry` method fills attribution fields from Worker identity. Same constitutional principle, more precise enforcement vehicle. UPDATE operations on already-attributed rows remain unaffected per the rule's statement.

  **Note (2026-07-27):** the sentence above previously named the rule's file path (`.intent/rules/architecture/blackboard.json`) explicitly. Removed per ADR-144 D2 row 14 (framework artifacts must not embed a path reference to a project-namespace artifact) — the rule ID already identifies it precisely and is the more durable reference.
