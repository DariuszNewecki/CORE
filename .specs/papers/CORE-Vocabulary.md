...
|------|-------------|----------------------|
| NorthStar | The reason CORE exists: use AI to write code in a controlled way. Law outranks intelligence. Defensibility outranks productivity. | `.specs/northstar/CORE - What It Does.md` |
...
| OptimizerWorker | An acting Worker. Observes repeated ViolationExecutor patterns and proposes AtomicAction codification for graduation to the constitutional path. Not yet designed. | `.specs/decisions/ADR-034-optimizer-worker-formal-deferral.md` |
...
| finalizing | The proposal lifecycle state entered after a proposal's changes are committed to git and held until its consequence chain is durably recorded; only on that record does the proposal become completed. | finalizing is not completed and not executing. It is the post-commit, evidence-recording state — the change is already applied but not yet defensible: no durable consequence record, deferred findings not yet resolved. A proposal stuck in finalizing is recovered by rolling forward — re-driving the idempotent evidence steps — never by rollback, which would double-apply the committed change. A proposal must not be reported as completed while any part of its consequence chain remains unrecorded. | `.specs/papers/CORE-Proposal.md` | | Proposal \| ProposalExecutor \| Crate |
...
| NorthStar | The reason CORE exists: use AI to write code in a controlled way. Law outranks intelligence. Defensibility outranks productivity. | NorthStar is not a strategy document or a feature roadmap. It is a governing law: the reference against which all CORE design decisions are evaluated. When a design decision conflicts with NorthStar, the decision changes, not the star. | `.specs/northstar/CORE - What It Does.md` | | Constitution \| Authority |
