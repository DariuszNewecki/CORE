...
- **F-27** Local LLM — **next.** Smallest finishing touch per §2.1. Promotion from `partial` to `shipping` requires a ≥7-day reliable local-LLM-only Solo run. The measurement window can start at any time; closure requires the window to complete.
