...
| ADR-050 | 2026-05-15 | CLI is outside CORE: operator client, not entry-point boundary | The closed system is `mind/` + `body/` + `will/`; `api/` is the sole governed entry point; `cli/` is an operator-facing client application, not a CORE layer. D2: the only permitted import target from `src/cli/` into the repository is `api.*` (third-party packages unrestricted). D3: Will may not import from CLI (5 reverse imports identified for migration). D4: a blocking `ast_gate import_boundary` rule enforces D2. Closes the CLI governance gap surfaced by the 2026-05-15 architecture audit. |
...
| ADR-054 | 2026-05-16 | API Phase 1 — `/audit` + `/proposals` | First capability cluster under ADR-053. D1: audit runs are resources (`POST /audit/runs`, `GET /audit/runs/{id}`). D2: `/proposals/{id}/execute` included in Phase 1 scope. D3: no auth — loopback binding only for Phase 1. D4: completion is verified by removing suppress entries for six CLI files (`proposals/{integrate,manage,list,create}.py`, `code/{audit,lint}.py`) until each reaches zero direct `mind/*`, `will/*`, `body/*`, `shared/*` imports. Closes #335. |
